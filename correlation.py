...
def pearson_similarity(movie_ratings, film1, film2):
    ...
    film1_sum = sum(int(film1_critics[critic]) for critic in common_critics)
    film2_sum = sum(int(film2_critics[critic]) for critic in common_critics)

    film1_sum_square  = sum([pow(int(film1_critics[critic]), 2) for critic in common_critics])
    film2_sum_square  = sum([pow(int(film2_critics[critic]), 2) for critic in common_critics])

    product_sum = sum([int(film1_critics[critic]) * int(film2_critics[critic]) for critic in common_critics])
